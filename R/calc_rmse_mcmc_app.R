...
calc_rmse_mcmc_app <- function(p_mcmc, as_mcmc, bs_mcmc, p0_mcmc, a0_mcmc, b0_mcmc, epsilon_mcmc, B_iter, fig_dir) {
  library(EnvStats)
  ...
  res_list <- get_DPpars_app(EXAMPLE_ID = EXAMPLE_ID)
  list2env(res_list, envir = globalenv())
  res_list <- get_HPpars_app(HP_ID = HP_ID)
  list2env(res_list, envir = globalenv())
  ...
  for (id_i in 1:K) {
    for (id_j in 1:K) {
      rmse_list[[(id_i - 1)*K + id_j]] <- (sweep(x = pos_ps_mixed[[(id_i - 1)*K + id_j]],
                                                 MARGIN = 2,
                                                 STATS = pos_ps_true_mixed[[(id_i - 1)*K + id_j]]))^2
    }
  }
  rmse_agg <- lapply(rmse_list, FUN = "colMeans")
  RMISE <- NA
  for (id_i in 1:K) {
    for (id_j in 1:K) {
      RMISE[(id_i - 1)*K + id_j] <- sum(sqrt(rmse_agg[[(id_i - 1)*K + id_j]])*PLOT_START)
    }
  }
