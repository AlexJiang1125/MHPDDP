...
calc_rmse_mcmc_meanspec <- function(p_mcmc, as_mcmc, bs_mcmc, p0_mcmc, a0_mcmc, b0_mcmc, epsilon_mcmc, B_iter, fig_dir) {
  library(EnvStats)
  ...
  for (j in 1:length(x_plot)) {
    a0_mcmc_mat <- matrix(rep(a0_mcmc[B_burn:B_iter], L), ncol = L, byrow= FALSE)
    pos_p0s[,j] <- eval_mixture_meanspec(x_plot[j],
  ...
  for (id_i in 1:K) {
    for (id_j in 1:K) {
      ...
      temp <- NA
      for (j in 1:length(x_plot)) {
        as_mcmc_mat = matrix(rep(as_mcmc[[(id_i - 1)*K + id_j]][B_burn:B_iter], L), ncol = L, byrow= FALSE)
        pos_ps[[(id_i - 1)*K + id_j]][,j] <- eval_mixture_meanspec(x_plot[j], p_mcmc[[(id_i - 1)*K + id_j]][B_burn:B_iter, ],
                                                          as_mcmc_mat,
