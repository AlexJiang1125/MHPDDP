...
calc_rmse_vi_app <- function(alpha0_vi, alphas_vi, cas_vi, cbs_vi, das_vi, dbs_vi,
                         ca0_vi, cb0_vi, da0_vi, db0_vi, eps_vi, type = "RANDOM",
                         fig_dir, ids, EXAMPLE_ID = EXAMPLE_ID) {
  library(EnvStats)
  res_list <- get_DPpars_app(EXAMPLE_ID = EXAMPLE_ID, T_kernel = T_kernel)
  list2env(res_list, envir = globalenv())
  res_list <- get_HPpars_app(HP_ID = HP_ID, filename = filename)
  list2env(res_list, envir = globalenv())
  # number of samples
  B_iter <- 5000
  ...
  for (j in 1:length(x_plot)) {
    pos_p0s[,j] <- eval_mixture(x_plot[j],p0m,a0m,b0m)
    pos_f0s[,j] <- eval_mixture(x_plot[j],p0m,a0m,b0m,type = "p")
  }
